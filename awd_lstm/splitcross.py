...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        ...
        # print('hiddens.shape: ' + str(hiddens.shape))
        ...
        for idx in range(self.nsplits):
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                tail_log_softmax = torch.nn.functional.log_softmax(tail_res, dim=-1)
                tail_entropy = torch.gather(tail_log_softmax, dim=1, index=indices).squeeze()
        ...
        return (total_loss / len(targets)).type_as(weight), loss_list
